...
from util import occupancy_reward_function, n_cars_reward_function, social_reward_function, speed_reward_function, \
    composite_reward_function, glob_outcome_reward_function, intergroup_outcome_reward_function,\
    document_episode, compute_jenson_shannon
...
class CustomEnvironment(Environment):
    def __init__(self,
        ...
        if self.document:
            ...
            self.nl.command(f'set output-file-path "{turtle_file_path}"')
        else:
    ...
    def states(self):
        if self.n_garages > 0:
            return dict(
                ...
                global_outcome_divergence=dict(type="float", min_value=0, max_value=1.0),
                intergroup_outcome_divergence=dict(type="float", min_value=0, max_value=1.0)
            )
        else:
            return dict(
                ticks=dict(type="float", min_value=0, max_value=21600),
    ...
    def execute(self, actions):
        ...
        if terminal and self.document:
            self.nl.command("ask cars [document-turtle]")  # ask remaining turtles to document
    ...
    def get_state(self):
        ...
        self.current_state['global_outcome_divergence'] = compute_jenson_shannon(self.nl, intergroup=False)
        self.current_state['intergroup_outcome_divergence'] = compute_jenson_shannon(self.nl, intergroup=True)
        ...
        state['global_outcome_divergence'] = np.float(self.current_state['global_outcome_divergence'])
        state['intergroup_outcome_divergence'] = np.float(self.current_state['intergroup_outcome_divergence'])
