...
def intergroup_outcome_reward_function(colours: List[str], current_state: Dict[str, float]):
    """
...
def document_episode(nl, path: Path, reward_sum, uuid):
    ...
    if turtle_csv_path.is_file():
        turtle_csv_path.rename(f"{episode_path}_turtles.csv")
...
def label_episodes(path: Path, df: pd.DataFrame, mode: str):
    ...
    :return:
    """
    episode_files = [fn for fn in glob(str(path) + "/E*.csv") if 'turtles' not in str(fn)]
    performances = dict()
    ...
    for metric in performances.keys():
        ...
        for episode in episode_files:
            ...
            if found:
                ...
                # Turtles csv
                os.rename(episode.replace(".csv", "_turtles.csv"),
                          str(new_path / f"{mode}_{metric}_{performances[metric]}_turtles.csv"))
                # View.png
...
def create_turtle_df(episode_path: str):
    ...
    data_df = data_df[
        ['who', 'income', 'income-grade', 'wtp', 'parking-offender?', 'distance-parking-target', 'price-paid',
         'search-time', 'park-time', 'park']]
...
def create_colourbar(fig):
    ...
    cbar.set_ticks([])
    cbar.ax.set_ylabel(r"$\Leftarrow$ Distance of CPZ to City Center", fontsize=25, loc="top")
